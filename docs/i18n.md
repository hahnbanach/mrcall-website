...
6. Create legal content files:
   - `content/legal/privacy/NEW.tsx` — translate from `en.tsx`, export default function `PrivacyNEW()`
   - `content/legal/terms/NEW.tsx` — translate from `en.tsx`, export default function `TermsNEW()`
   - Add dynamic import entries in `app/[locale]/privacy/page.tsx` and `app/[locale]/terms/page.tsx`

7. Build and verify: `npm run build` — should generate new static pages for the locale.
